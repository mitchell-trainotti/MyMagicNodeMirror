async function initialize(){ //get Data and set values
	...
	//Sunrise text update
	document.getElementById('Sunrise_Text').innerHTML = GetSun(json)[0];
	var offset = json.offset; //offset indicates the offset in time from London
	var sunsetSec = json.daily.data[0].sunsetTime+offset*3600;
	var sunriseSec = json.daily.data[0].sunriseTime+offset*3600; //adding 7 hours (from London) in seconds.
	sunriseSec = sunriseSec%86400; //removing the days and getting it down to hours.
	sunsetSec = sunsetSec%86400; //removing the days and getting it down to hours.
	...
	document.getElementById('LowHi_Text').innerHTML = GetLowHi(TemperatureLow, TemperatureHi);
...
function GetLowHi(TemperatureLow, TemperatureHi){	
